...
typedef struct
{
    bool (*init)(BspSPI_Config_TypeDef spi_cfg, void *spi_handle);
    bool (*deinit)(BspSPI_Config_TypeDef spi_cfg);
    bool (*trans)(void *spi_handle, uint8_t *tx, uint16_t size, uint16_t time_out);
    bool (*receive)(void *spi_handle, uint8_t *rx, uint16_t size, uint16_t time_out);
    uint16_t (*trans_receive)(void *spi_handle, uint8_t *tx, uint8_t *rx, uint16_t size, uint16_t time_out);
    bool (*set_speed)(void *spi_handle, uint32_t speed);
} BspSpi_TypeDef;
