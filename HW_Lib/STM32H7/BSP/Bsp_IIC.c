    ...
    if(obj && obj->Pin->port_sck && obj->Pin->port_sda)
    {
        ...
        switch((uint8_t)(obj->instance_id))
        {
            case BspIIC_Instance_I2C_2:
                ...
                To_IIC_PeriphCLKInitType(obj->PeriphClkInitStruct)->I2c123ClockSelection = RCC_I2C123CLKSOURCE_PLL3;
...
static bool BspIIC_Read(BspIICObj_TypeDef *obj, uint16_t dev_addr, uint16_t reg, uint8_t *p_buf, uint16_t len)
{
    if(obj && p_buf && len)
    {
        if(HAL_I2C_Mem_Read(To_IIC_Handle_Ptr(obj->handle), dev_addr, reg, I2C_MEMADD_SIZE_8BIT, p_buf, len, 100) == HAL_OK)
            return true;
    }
...
static bool BspIIC_Write(BspIICObj_TypeDef *obj, uint16_t dev_addr, uint16_t reg, uint8_t *p_buf, uint16_t len)
{
    if(obj && p_buf && len)
    {
        if(HAL_I2C_Mem_Write(To_IIC_Handle_Ptr(obj->handle), dev_addr, reg, I2C_MEMADD_SIZE_8BIT, p_buf, len, 100) == HAL_OK)
            return true;
    }
