...
{
    if (p_data && len)
        BspUart.send(To_BspUart_ObjPtr(Obj->port_obj), p_data, len);
}

void Debug_Port_Init(DebugPrintObj_TypeDef *Obj)
{
    To_BspUart_ObjPtr(Obj->port_obj)->hdl = Obj->malloc(UART_HandleType_Size);
    if (To_BspUart_ObjPtr(Obj->port_obj)->hdl == NULL)
        return;

    if (!Obj->port_obj || !BspUart.init(To_BspUart_ObjPtr(Obj->port_obj)))
    {
        Obj->port_obj = NULL;
        return;
    }
